 ...
 *
 * Ask the model, surface its text as it arrives, run the tools it calls,
 * replay the conversation with the results, repeat until it stops. This
 * module is that machine; the agent supplies the prompt, the tools, and what
 * to do when a tool is called.
 *
 ...
 * the SDK's own tool runner deliberately does not offer: a two-way generator
 * that yields AgentStreamEvents and takes a ToolReply back through next().
 * The runner wants tools as callbacks it owns; these tools have to run in
 * the caller, where the document state lives and where each call also emits
 * a UI event.
 */
...
export async function* driveClaudeAgent(
  ...
  // is documented as always last, and the agent emits its terminal `done`
  ...
  for (let turn = 0; turn < opts.maxTurns; turn++) {
    ...
    // Tools run once the turn is complete, never mid-stream: a tool that
    // waits on a human would otherwise hold the HTTP connection open.
    ...
    switch (message.stop_reason) {
      ...
      case 'pause_turn':
        ...
        // assistant message is handed straight back. Unreachable while only
        // our own tools are declared, but "unreachable" and "discards a
...
/**
 * Apply an exact-match text edit, the way the edit_file tool does.
 *
