 ...
 *
 * Everything this package asks Claude goes through a client built here, so
 * the two things that are true of every call are stated once.
 */
...
/**
 * The SDK is fetched the first time something asks Claude, not when the host
 * loads. Translating is an occasional act; a dynamic import
 * keeps their transport out of a host's initial bundle and in a chunk of
 * its own.
 */
 ...
 *
 * This translator is meant to run where the documents are edited, which may
 * be a browser with no server behind it: the key is the editor's own, held
 * by them, sent straight to the API. Hence direct browser access is allowed.
 * In Node the flag is harmless.
 */
export async function claudeClient(apiKey: string): Promise<AnthropicClient> {
  ...
  // A content editor mid-translation deserves the sentence below instead.
  if (!apiKey) {
    throw new Error('No Claude API key is set. Provide one to translate.');
  }
